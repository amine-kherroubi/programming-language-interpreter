...
class SyntacticAnalyzer:
    ...
    def _peek_next_token(self) -> Token:
        pos: int = self._lexical_analyzer.position
        char: Optional[str] = self._lexical_analyzer.current_char
        line: int = self._lexical_analyzer.line
        col: int = self._lexical_analyzer.column
    ...
    def _parameter(self) -> NodeParameter:
        ...
        param_type: NodeType = self._type()
    ...
    def _primary_expression(self) -> NodeArithmeticExpression:
        ...
        if token.type == TokenType.IDENTIFIER:
            next_token: Token = self._peek_next_token()
